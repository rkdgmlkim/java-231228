...
// : 클래스의 동작을 변경하지 않고 (오버라이딩) 안정성을 보장하기 위함
// 기본 형식 class 키워드 앞에 final 키워드 작성

final class MyFinalClass {
	// 2. final 메소드
	// : 자식 클래스에서 오버라이딩 할 수 없는 메소드를 의미
	// : final 클래스의 모든 메소드는 암묵적으로 final
	// : 메소드의 동작을 변경하지 못하도록 지정
	public final void MyMethod() {
		// public + final >> private 메소드와 유사하게 동작
	}
	
	// 3. final 변수
	// : 상수를 지정하는 키워드
	// : 재할당이 불가능한 변수, 선언과 동시에 초기화
	// : 변수명 모두 대문자로 작성, 단어와 단어의 연결을 (_) 기호로 연결
	
	final int FINAL_VARIABLE=30;
//	FINAL_VARIABLE = 10; - Error
}
...
public class C_Final {
	...
	// 장점
	// 불변성 : final 키워드를 통해 객체의 상태를 변경하지 못하게 ㅎㅁ
	// - 안정성 보장을 위해 사용
